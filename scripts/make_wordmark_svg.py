...
TEXT = os.environ.get("WORDMARK_TEXT", "YUUTO")


def resolve_font_path(path):
    """Return a usable font file path or None.

    If the configured path doesn't exist, search common system font folders for
    likely fallbacks (Futura, DejaVu, Arial, Segoe, Calibri, Roboto). On
    Windows this will typically find arialbd.ttf or Segoe UI Semibold.
    """
    if path and os.path.isfile(path):
        return path
    envp = os.environ.get("WORDMARK_FONT")
    ...
    if os.name == "nt":
        ...
        try:
            for fn in os.listdir(fonts_dir):
                n = fn.lower()
                if any(k in n for k in ["futura", "dejavu", "arial", "segui", "calibri", "roboto"]):
                    candidates.append(os.path.join(fonts_dir, fn))
        except Exception:
            pass
    else:
        for d in ["/System/Library/Fonts", "/usr/share/fonts", "/usr/local/share/fonts"]:
            if os.path.isdir(d):
                try:
                    for fn in os.listdir(d):
                        n = fn.lower()
                        if any(k in n for k in ["futura", "dejavu", "arial", "roboto"]):
...
def build_shell():
    ...
    # resolve a usable font file on the current system; on macOS the default
    # path works, but on Windows the shipped path does not. If resolution fails,
    # print a helpful error and exit so the user can set WORDMARK_FONT.
    local_font = resolve_font_path(FONT_PATH)
    if not local_font:
        raise SystemExit(
            "could not find a usable TTF/OTF font. Set WORDMARK_FONT env to a path to a TrueType font (e.g. C:\\Windows\\Fonts\\arialbd.ttf)"
        )

    for _ in range(40):                       # shrink until one line fits a sane raster
        try:
            font = ImageFont.truetype(local_font, font_size, index=FONT_INDEX)
        except OSError:
            # if index is wrong for this file, retry without index
            try:
                font = ImageFont.truetype(local_font, font_size)
            except OSError as e:
                raise SystemExit(f"failed to load font '{local_font}': {e}")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">yuutokata@github: ~$ ./wordmark.sh --3d</text>')
